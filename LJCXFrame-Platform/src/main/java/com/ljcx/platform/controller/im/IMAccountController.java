...
public class IMAccountController {
    ...
    public ResponseInfo userSign(@RequestBody String info){
        JSONObject jsonObject = JSONObject.parseObject(info);
        if(jsonObject == null || jsonObject.getInteger("isFail") == null){
            return new ResponseInfo().failed("isFail发送状态不能为空");
        }
        UserBaseBean baseBean = getUserSign(jsonObject.getInteger("isFail"), UserUtil.getCurrentUser().getId());
    ...
    @PostMapping("/import")
    public ResponseInfo accountImport() {
        UserBaseBean user = userBaseService.getById(UserUtil.getCurrentUser().getId());
