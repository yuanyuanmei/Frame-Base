...
function addMsg(newMsg) {
    ...
    if($(".dmMsg").length > 0) {
        if(newMsg.fromAccount == oIndex.currentUser.username){
    ...
    var msgDiv = "<div class=\"dmMsg clearfix  "+info_align+"\">\n"+
        "<div class=\"infor-col2 clearfix\">\n" +
        "                        <span class=\"name\">"+newMsg.fromAccountNick+"</span>\n" +
        "                        <span class=\"time\">"+oIndex.getMyDate(newMsg.time*1000)+"</span>\n" +
        "                    </div>\n" +
        MsgBody(newMsg.elems)+
        "         </div>";

    $(".infor-list").append(msgDiv);


    var nscrollHeight = $(".infor-list")[0].scrollHeight;
    var nHeight = $(".infor-list").outerHeight();
    var st1 = (nscrollHeight >= nHeight) ? nscrollHeight - nHeight : 0;

    $('.infor-list').animate({scrollTop:st1},'slow');
}

function MsgBody(MsgBody){
    var msgDiv = "";
    for(var i = 0;i<MsgBody.length;i++){
        if(MsgBody[i].type == "TIMTextElem"){
            msgDiv += "                    <div class=\"infor-col3 clearfix\">\n" +
                "                        <p>"+MsgBody[i].content.text+"</p>\n" +
                "                    </div>";
        }else if(MsgBody[i].type == "TIMImageElem"){
            /*msgDiv += "<div class=\"infor-col3 clearfix\">\n" +
                "                        <p><img src="+MsgBody[i].content.ImageInfoArray[2].url+" /></p>\n" +
                "                    </div>";*/
            msgDiv += "<div class=\"dialog\">\n" +
                "                        <div class=\"imgDiv\">\n" +
                "                            <img src=\""+ MsgBody[i].content.ImageInfoArray[2].url +"\">\n" +
                "                        </div>\n" +
                "                        <img class=\"img2\" src=\""+ MsgBody[i].content.ImageInfoArray[2].url +"\">\n" +
                "                    </div>";
        }else if(MsgBody[i].type == "TIMSoundElem"){
            msgDiv += "<div class=\"infor-col3 clearfix\">\n" +
                "                        <p><audio src=" + MsgBody[i].content.downUrl + " controls=\"controls\" onplay=\"onChangePlayAudio(this)\" preload=\"none\"></audio></p>\n" +
                "                    </div>";
        }
    }
    return msgDiv;
